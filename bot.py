...
FIPI_GEO_OGE_CSV = "/root/pro-lesson-data/fipi/geography/oge/tasks_geography_oge.csv"

def load_fipi_tasks_geography_oge():
    tasks = []
    try:
        with open(FIPI_GEO_OGE_CSV, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f, delimiter=";")
            for row in reader:
                tasks.append(row)
    except Exception as e:
        logger.error(f"FIPI CSV error: {e}")
    return tasks
...
def get_fipi_task_geography_oge(kim_number=None, difficulty=None, topic=None):
    ...
    for t in tasks:
        if kim_number is not None and str(t.get("kim_number")) != str(kim_number):
            continue
        if difficulty is not None and str(t.get("difficulty")) != str(difficulty):
            continue
...
def get_tariff(user_id):
    return TARIFFS.get(get_user_data(user_id)["tariff"], TARIFFS["demo"])

def check_query_limit(user_id):
    d = get_user_data(user_id)
    t = TARIFFS.get(d["tariff"], TARIFFS["demo"])
...
def use_query(user_id):
    d = get_user_data(user_id)
    d["queries_used"] += 1
    update_user_data(user_id, d)

def use_generation(user_id):
    d = get_user_data(user_id)
    d["generations_used"] += 1
    update_user_data(user_id, d)
...
SYSTEM_PROMPT = f"""Ты — {BOT_NAME}, профессиональный ИИ-помощник учителя.
Помогаешь учителям 5–11 классов любых предметов быстро и качественно готовиться к урокам.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ЧТО ТЫ УМЕЕШЬ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

• Планы уроков — полные технологические карты по ФГОС
• Конспекты и рабочие листы для учеников
• Тесты, контрольные, проверочные работы (в том числе в формате ОГЭ/ЕГЭ)
• КТП (календарно-тематическое планирование)
• Рабочие программы и пояснительные записки
• Материалы для внеурочной деятельности
• Разборы и объяснения тем — для учителя и для учеников
• Анализ фото (рукописных работ, схем, таблиц)
• Идеи уроков, проектов, исследовательских работ

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
УЧЕБНЫЕ ПРЕДМЕТЫ (актуальные названия по ФГОС 2021)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Русский язык, Литература
Родной язык, Родная литература
Иностранный язык, Второй иностранный язык
Математика (курсы: Алгебра, Геометрия, Вероятность и статистика)
Информатика
История (курсы: История России, Всеобщая история, История нашего края)
Обществознание
География
Физика, Химия, Биология
Изобразительное искусство, Музыка
Труд (технология)   ← именно так, НЕ «Технология»
Основы безопасности и защиты Родины   ← НЕ «ОБЖ»
Физическая культура
Духовно-нравственная культура России   ← вводится с 2026/27 уч. года

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
НОРМАТИВНАЯ БАЗА — ФГОС ООО (Приказ №287 от 31.05.2021)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ГЛАВНЫЙ ПРИНЦИП: системно-деятельностный подход.
Ученик — субъект обучения, а не объект. Учитель организует деятельность, а не транслирует знания.

─── ПЛАНИРУЕМЫЕ РЕЗУЛЬТАТЫ — три обязательные группы ───

1. ЛИЧНОСТНЫЕ РЕЗУЛЬТАТЫ — 8 направлений воспитания:
   • Гражданское воспитание: готовность к выполнению обязанностей гражданина, неприятие экстремизма, участие в жизни общества
   • Патриотическое воспитание: осознание российской идентичности, уважение к символам России, достижениям Родины
   • Духовно-нравственное воспитание: ориентация на моральные ценности, неприятие асоциальных поступков
   • Эстетическое воспитание: восприимчивость к искусству, стремление к самовыражению
   • Физическое воспитание: ценность жизни и здоровья, ЗОЖ, неприятие вредных привычек
   • Трудовое воспитание: уважение к труду, интерес к профессиям, готовность к самостоятельной деятельности
   • Экологическое воспитание: ориентация на применение знаний для решения экологических задач
   • Ценность научного познания: ориентация на научные представления, овладение навыками исследования

2. МЕТАПРЕДМЕТНЫЕ РЕЗУЛЬТАТЫ — три группы УУД:

   Познавательные УУД:
   • Базовые логические действия: анализ, сравнение, классификация, выявление причинно-следственных связей, построение умозаключений
   • Базовые исследовательские действия: формулировка гипотез, планирование опытов, оценка достоверности, выводы
   • Работа с информацией: поиск, отбор, анализ, систематизация, представление информации в разных форматах

   Коммуникативные УУД:
   • Общение: формулировка суждений, аргументация, ведение диалога и дискуссии, публичные выступления
   • Совместная деятельность: распределение ролей, командная работа, оценка вклада каждого

   Регулятивные УУД:
   • Самоорганизация: постановка целей, планирование, выбор способа решения
   • Самоконтроль: рефлексия, оценка результата, корректировка действий
   • Эмоциональный интеллект: управление эмоциями, понимание других, принятие себя

3. ПРЕДМЕТНЫЕ РЕЗУЛЬТАТЫ:
   • Формулируются в деятельностной форме («умеет», «знает», «применяет»)
   • Конкретны для каждого предмета и класса
   • Ориентированы на применение в учебных и жизненных ситуациях

─── СТРУКТУРА УРОКА (технологическая карта) ───

Обязательные этапы по ФГОС:
1. Мотивация / организационный момент — пробуждение интереса, постановка проблемы
2. Актуализация знаний — связь с предыдущим материалом, опорные знания
3. Открытие нового знания — самостоятельное «добывание» учеником через деятельность
4. Первичное закрепление — применение в типовых ситуациях
5. Самостоятельная работа с самопроверкой — контроль усвоения
6. Включение в систему знаний — связь с другими темами, межпредметность
7. Рефлексия — оценка своей деятельности, достижения целей

─── ОЦЕНИВАНИЕ ───

По ФГОС используются разнообразные формы:
• Проекты (индивидуальные и групповые)
• Практические и исследовательские работы
• Творческие и командные задания
• Самооценка и взаимооценка
• Наблюдение и тестирование
• Динамическое оценивание (портфолио, рост показателей)

─── РАБОЧАЯ ПРОГРАММА — обязательная структура ───

Каждая рабочая программа должна содержать:
1. Содержание учебного предмета (по темам и разделам)
2. Планируемые результаты (личностные, метапредметные, предметные)
3. Тематическое планирование с указанием:
   • часов на каждую тему
   • возможности использования цифровых образовательных ресурсов (ЦОР)
Рабочие программы формируются с учётом рабочей программы воспитания.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ВАЖНЫЕ ИЗМЕНЕНИЯ — ПРИКАЗ №110 от 19.02.2024 (действует с 01.09.2025)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. ИСТОРИЯ — новый обязательный курс «История нашего края»:
   Предмет История теперь включает три курса:
   ✓ История России
   ✓ Всеобщая история
   ✓ История нашего края (НОВЫЙ — обязателен с 01.09.2025)

   Планируемые результаты курса «История нашего края»:
   • Понимание вклада народов России в формирование цивилизационного наследия
   • Понимание ценности многообразия культурных укладов народов РФ
   • Интерес к традициям собственного народа и народов России
   • Знание исторических примеров взаимопомощи народов РФ

   При составлении планов уроков по истории — ВСЕГДА учитывай региональный компонент.

2. ИСТОРИЯ — уточнение формулировки предметного результата (п. 45.6.1, пп. 1):
   Новая формулировка: «умение определять последовательность событий, явлений, процессов;
   соотносить события истории разных стран и народов с историческими периодами,
   событиями региональной, отечественной и мировой истории»

3. МАТЕМАТИКА — три обязательных курса:
   Предмет Математика включает:
   ✓ Алгебра
   ✓ Геометрия
   ✓ Вероятность и статистика
   При планировании урока по математике — указывай конкретный курс.

4. «Духовно-нравственная культура России» — новый предмет с 2026/27 уч. года.
   Направлен на изучение традиционных российских ценностей через биографии выдающихся людей.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА — НАРУШАТЬ НЕЛЬЗЯ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⛔ ЗАПРЕЩЁННЫЕ НАЗВАНИЯ ПРЕДМЕТОВ:
   НЕЛЬЗЯ писать «ОБЖ» — ТОЛЬКО «Основы безопасности и защиты Родины»
   НЕЛЬЗЯ писать «Технология» — ТОЛЬКО «Труд (технология)»
   Если учитель написал «ОБЖ» или «технология» — мягко используй правильное название в ответе.

⛔ МАТЕМАТИКА — ОБЯЗАТЕЛЬНОЕ УТОЧНЕНИЕ:
   Если учитель просит что-то по «математике» и НЕ указал конкретный курс —
   СНАЧАЛА уточни: «Уточните, пожалуйста, по какому курсу: Алгебра, Геометрия или Вероятность и статистика?»
   НЕЛЬЗЯ сразу делать материал по алгебре или любому другому курсу без уточнения.

⛔ ИСТОРИЯ — ОБЯЗАТЕЛЬНОЕ УПОМИНАНИЕ ТРЁХ КУРСОВ:
   Предмет «История» с 01.09.2025 включает ТРИ курса:
   ✓ История России
   ✓ Всеобщая история
   ✓ История нашего края
   При любом запросе по истории — ВСЕГДА упоминай все три курса.
   При составлении плана урока по истории — ОБЯЗАТЕЛЬНО включи региональный компонент
   (связь темы с историей своего края / региона).

⛔ ГЕОГРАФИЯ — ОБЯЗАТЕЛЬНАЯ СВЯЗЬ С КРАЕВЕДЕНИЕМ:
   При любом запросе по географии — ВСЕГДА добавляй региональный / краеведческий компонент:
   как тема урока связана с географией своего региона, края, области.
   Это требование ФГОС (связь с местным сообществом и окружающей средой).

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
КАК РАБОТАТЬ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

• Отвечай по-русски, развёрнуто, практично
• Всегда уточняй класс и тему, если не указаны — спроси кратко
• При составлении плана урока — ВСЕГДА указывай планируемые результаты в трёх группах
• Документы (Word, PowerPoint, Excel) создавай только по запросу через кнопки меню
• В обычных ответах НЕ используй маркеры [PPTX_DOC], [WORD_DOC], [EXCEL_DOC]

Полезные источники: resh.edu.ru, yaklass.ru, infourok.ru, fipi.ru
"""
...
HTML_TEST_PROMPT = """ВАЖНО: Ты ОБЯЗАН вернуть ТОЛЬКО HTML код. Никакого текста, никаких пояснений, никакого markdown.

Начни ответ РОВНО с [HTML_TEST]<!DOCTYPE html> и дальше только HTML.

Создай полноценный интерактивный HTML тест. Внутри одного файла: HTML + CSS в <style> + JavaScript в <script>.

СТРУКТУРА:
1. Экран ввода ФИО ученика (имя, фамилия, класс) + кнопка "Начать тест"
2. Экран с вопросами (по одному, 10-12 вопросов, 4 варианта)
3. При клике на вариант: правильный — зелёный фон + галочка, неправильный — красный фон + показать правильный
4. Кнопка "Далее" появляется ПОСЛЕ выбора ответа
5. Экран результатов: ФИО, балл X из Y, процент, оценка (5/4/3/2), кнопка "Пройти заново"

ДИЗАЙН (CSS):
- body: background gradient #667eea to #764ba2, font-family: system-ui, sans-serif
- Карточка: белый фон, border-radius 20px, box-shadow, padding 40px, max-width 600px, margin auto
- Вопрос: font-size 1.2em, font-weight 600, margin-bottom 20px
- Варианты: кнопки на всю ширину, padding 15px, border-radius 12px, border: 2px solid #e0e0e0, cursor pointer, margin 8px 0, transition 0.3s
- Правильный: background #d4edda, border-color #28a745
- Неправильный: background #f8d7da, border-color #dc3545
- Кнопка Далее: background #667eea, color white, padding 12px 40px, border-radius 10px, font-size 1.1em
- Прогресс-бар сверху карточки
- Номер вопроса: "Вопрос 3 из 12"
...
def preprocess_message(text):
    """
    Предобработка запроса до отправки в Claude.
    Возвращает (modified_text, early_reply):
      - modified_text: текст для Claude (с исправлениями названий + доп. инструкции)
      - early_reply: готовый ответ пользователю (или None — идём в Claude)
    """
    tl = text.lower()

    # ── Математика: уточнение курса ──────────────────────────
    math_words = ['математик', 'по матем', 'урок матем', 'контрольн',
                  'тест по матем', 'план урока по матем', 'кр по матем']
    ...
    if any(w in tl for w in math_words) and not any(w in tl for w in course_words):
        return text, (
            "Уточните, пожалуйста, по какому курсу математики:\n\n"
            "• Алгебра\n"
            "• Геометрия\n"
            "• Вероятность и статистика\n\n"
            "Например: «контрольная по алгебре, 8 класс»"
        )

    # ── Автозамена названий предметов ────────────────────────
    modified = text
    # ОБЖ → Основы безопасности и защиты Родины
    modified = re.sub(r'(?i)\bобж\b', 'Основы безопасности и защиты Родины', modified)
    # Технология → Труд (технология)
    modified = re.sub(r'(?i)\bтехнологи[яиюей]\b', 'Труд (технология)', modified)

    # ── История: добавляем инструкцию о трёх курсах ──────────
    history_words = ['истори', 'по истор', 'урок истор', 'план истор',
                     'конспект по истор', 'тест по истор']
    if any(w in tl for w in history_words):
        modified += (
            "\n\n[ВАЖНО: Предмет История включает ТРИ обязательных курса: "
            "1) История России, 2) Всеобщая история, 3) История нашего края (с 01.09.2025). "
            "Обязательно упомяни все три курса в ответе. "
            "Добавь раздел про региональный компонент — связь темы с историей своего края/региона.]"
        )

    # ── География: добавляем краеведческий компонент ─────────
    geo_words = ['географи', 'по геогр', 'урок геогр', 'план геогр',
                 'конспект по геогр', 'тест по геогр']
    if any(w in tl for w in geo_words):
        modified += (
            "\n\n[ВАЖНО: По требованиям ФГОС обязательно добавь краеведческий компонент — "
            "как данная тема связана с географией своего региона/края/области. "
            "Выдели это в отдельный раздел плана урока.]"
        )
...
def clean_for_chat(text):
    c = text
    for m in ['[PPTX_DOC]', '[WORD_DOC]', '[EXCEL_DOC]', '[HTML_TEST]']:
        c = c.replace(m, '')
    c = re.sub(r'\[/?SLIDE\]', '', c)
    c = re.sub(r'\[CHART\].*?\[/CHART\]', '', c, flags=re.DOTALL)
...
def choose_theme(t):
    tl = t.lower()
    if any(w in tl for w in ['матем', 'физик', 'информ', 'алгебр']):
        return THEMES["blue"]
    elif any(w in tl for w in ['биолог', 'природ', 'географ']):
        return THEMES["green"]
    elif any(w in tl for w in ['истор', 'литератур', 'музык']):
        return THEMES["warm"]
    return THEMES["blue"]
...
def parse_chart(text):
    ...
    for m in re.findall(r'\[CHART\](.*?)\[/CHART\]', text, re.DOTALL):
        ...
        for l in m.strip().split('\n'):
            l = l.strip()
            if l.startswith('ТИП:'):
                c['type'] = l[4:].strip().lower()
            elif l.startswith('НАЗВАНИЕ:'):
                c['title'] = l[9:].strip()
            elif l.startswith('МЕТКИ:'):
                c['labels'] = [x.strip() for x in l[6:].split(',')]
...
def parse_image(text):
    ...
    for m in re.findall(r'\[IMAGE\](.*?)\[/IMAGE\]', text, re.DOTALL):
        ...
        for l in m.strip().split('\n'):
            l = l.strip()
            if l.startswith('ЗАПРОС:'):
                img['query'] = l[7:].strip()
            elif l.startswith('ОПИСАНИЕ:'):
                img['description'] = l[9:].strip()
        if img['query']:
            imgs.append(img)
    return imgs
    ...
    for part in content.split('[SLIDE]'):
        ...
        for l in clean.split('\n'):
            ...
            if s.startswith('ЗАГОЛОВОК:'):
                sl['title'] = s[10:].strip()
            elif s.startswith('ПОДЗАГОЛОВОК:'):
                sl['subtitle'] = s[13:].strip()
            elif s.startswith('СОДЕРЖИМОЕ:'):
                sec = 'content'
            elif s.startswith('ЗАМЕТКИ:'):
                sl['notes'] = s[8:].strip()
                sec = 'notes'
            elif sec == 'content' and s:
                sl['content'].append(s[2:] if s[:2] in ['- ', '• '] else s)
            elif sec == 'notes' and s:
                sl['notes'] += ' ' + s
        if sl['title']:
            slides.append(sl)
    return slides
        ...
        if sdata.get('charts'):
            ...
            if img:
                try:
                    ...
                    cp.text = sdata['images'][0].get('description', '')
                    cp.font.size = PptxPt(10)
                    cp.font.italic = True
    ...
    if tl:
        for line in tl:
            ...
            for ci, v in enumerate(cells, 1):
                ...
                cell.border = bd
                if rn == 1:
                    cell.font, cell.fill, cell.alignment = hf, hfl, ha
                else:
...
async def generate_document(update, context, uid, fmt, topic=None):
    ...
    if dt == 'pptx':
        ...
        await update.message.reply_document(
            ...
            caption="Интерактивный тест! Откройте файл в браузере на телефоне или компьютере.",
        )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    if q.data in BUTTON_PROMPTS:
        await q.message.reply_text(BUTTON_PROMPTS[q.data])
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await safe_reply(update, "Ошибка.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sd:
        ...
        use_query(uid)
        answer = ask_claude(uid, f"Документ {dn}:\n\n{sd}\n\n{text}")
        await safe_reply(update, clean_for_chat(answer))
        return
    ...
    if sp:
        ...
        await safe_reply(update, "Анализирую...")
        use_query(uid)
        answer = ask_claude(uid, f"Изображение. {text}", image_data=sp)
        await safe_reply(update, clean_for_chat(answer))
        return

    tl = text.lower()
    if any(kw in tl for kw in ['диаграмм', 'график', 'построй', 'визуализ', 'столбчат', 'круговую']):
        use_query(uid)
        chart_prompt = text + """
Ответь в формате для построения диаграммы. Используй свои знания для данных.
ОБЯЗАТЕЛЬНО включи блок:
[CHART_DATA]
...
Перед блоком напиши краткое пояснение к данным. После блока ничего не пиши.
"""
        ...
        if chart_match:
            ...
            for l in chart_match[0].strip().split('\n'):
                l = l.strip()
                if l.startswith('ТИП:'):
                    cd['type'] = l[4:].strip().lower()
                elif l.startswith('НАЗВАНИЕ:'):
                    cd['title'] = l[9:].strip()
                elif l.startswith('МЕТКИ:'):
                    cd['labels'] = [x.strip() for x in l[6:].split(',')]
                elif l.startswith('ЗНАЧЕНИЯ:'):
                    try:
